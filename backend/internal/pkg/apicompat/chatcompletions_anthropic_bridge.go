...
func chatAnthropicTextFromBlocks(blocks []AnthropicContentBlock) string {
...
// AnthropicStopReasonPtr and AnthropicStopReasonString retain source compatibility
// for bridge callers while stop reasons are represented as strings.
func AnthropicStopReasonPtr(reason string) string { return reason }

func AnthropicStopReasonString(reason string) string { return reason }
...
func anthropicAssistantToChatMessages(raw json.RawMessage) ([]ChatMessage, error) {
	...
	text := chatAnthropicTextFromBlocks(blocks)
	if text != "" {
		content, _ := json.Marshal(text)
		msg.Content = content
	}
