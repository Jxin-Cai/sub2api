		...
		if s.settingService != nil && s.settingService.IsModelFallbackEnabled(ctx) &&
			...
			if fallbackModel != "" && fallbackModel != mappedModel {
				...
				if err == nil {
					...
					if err == nil {
						fallbackResp, err := s.httpUpstream.DoWithTLS(fallbackReq, proxyURL, account.ID, account.Concurrency, tlsfingerprint.AntigravityRealProfile)
...
func (s *AntigravityGatewayService) ForwardUpstream(ctx context.Context, c *gin.Context, account *Account, body []byte) (*ForwardResult, error) {
	...
	// 发送请求
	resp, err := s.httpUpstream.DoWithTLS(req, proxyURL, account.ID, account.Concurrency, tlsfingerprint.AntigravityRealProfile)
	if err != nil {
