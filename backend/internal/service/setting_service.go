...
func (s *SettingService) GetAllowedIframeOrigins(ctx context.Context) ([]string, error) {
	settings, err := s.GetPublicSettings(ctx)
	if err != nil {
		return nil, err
	}
...
func (s *SettingService) GetFrameSrcOrigins(ctx context.Context) ([]string, error) {
	return s.GetAllowedIframeOrigins(ctx)
}
...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	updates[SettingKeyOpenAIModelPriorityRules] = MarshalOpenAIModelPriorityRules(settings.OpenAIModelPriorityRules)
...
func (s *SettingService) refreshCachedSettings(settings *SystemSettings) {
	...
	openAIModelPriorityRulesSettingSF.Forget(SettingKeyOpenAIModelPriorityRules)
	openAIModelPriorityRulesSettingCache.Store(&cachedOpenAIModelPriorityRulesSetting{
		rules:     cloneOpenAIModelPriorityRules(settings.OpenAIModelPriorityRules),
		expiresAt: time.Now().Add(openAIAdvancedSchedulerSettingCacheTTL).UnixNano(),
	})
	if s.onUpdate != nil {
...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyOpenAIModelPriorityRules:       "[]",
	}
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result.OpenAIModelPriorityRules = ParseOpenAIModelPriorityRules(settings[SettingKeyOpenAIModelPriorityRules])
